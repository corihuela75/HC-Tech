...
export const createEmpresa = async (data) => {
    const { nombre, pag_web,user_id, direccion, razon_social, created_at, email, telefono, imagen, cuit } = data;

  const [result] = await pool.query('INSERT INTO empresas (nombre,user_id, pag_web, direccion, telefono, email, razon_social, created_at, imagen, cuit) VALUES (?, ?, ?, ?, ?,?, ?, ?, ?, ?)', [
    nombre,
    user_id,
    pag_web,
    direccion,
...
export const updateEmpresa = async (data) => {
  const { id, nombre, pag_web, direccion, razon_social, created_at, email, telefono, imagen, cuit } = data;
